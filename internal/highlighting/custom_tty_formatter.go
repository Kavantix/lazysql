...
func styleToEscapeSequence(table *ttyTable, style *chroma.Style, selected bool) map[chroma.TokenType]string {
	style = clearBackground(style, selected)
...
func clearBackground(style *chroma.Style, selected bool) *chroma.Style {
...
type customTTYFormatter struct {
	table          *ttyTable
	SelectionStart int
	SelectionEnd   int
}

func write(w io.Writer, theme map[chroma.TokenType]string, token chroma.Token) {
...
func (c *customTTYFormatter) Format(w io.Writer, style *chroma.Style, it chroma.Iterator) (err error) {
	normalTheme := styleToEscapeSequence(c.table, style, false)
	selectedTheme := styleToEscapeSequence(c.table, style, true)
	index := 0
	for token := it(); token != chroma.EOF; token = it() {
		nextIndex := index + len(token.Value)
		if index+len(token.Value) >= c.SelectionStart && index < c.SelectionEnd {
			tokenValue := token.Value
			lenToSkip := 0
			if index < c.SelectionStart {
				lenToSkip = c.SelectionStart - index
				index = c.SelectionStart
				token.Value = tokenValue[:lenToSkip]
				write(w, normalTheme, token)
			}
			lenToSelect := c.SelectionEnd - index
			if lenToSelect > len(tokenValue)-lenToSkip {
				lenToSelect = len(tokenValue) - lenToSkip
			}
			token.Value = tokenValue[lenToSkip : lenToSkip+lenToSelect]
			write(w, selectedTheme, token)
			if lenToSkip+lenToSelect < len(tokenValue) {
				token.Value = tokenValue[lenToSkip+lenToSelect:]
				write(w, normalTheme, token)
			}
		} else {
			write(w, normalTheme, token)
...
// var TTY = Register("terminal", &customTTYFormatter{ttyTables[8]})

// TTY8 is an 8-colour terminal formatter.
//
// The Lab colour space is used to map RGB values to the most appropriate index colour.
// var TTY8 = Register("terminal8", &customTTYFormatter{ttyTables[8]})
...
//
// The Lab colour space is used to map RGB values to the most appropriate index colour.
// var TTY16 = Register("terminal16", &customTTYFormatter{ttyTables[16]})
